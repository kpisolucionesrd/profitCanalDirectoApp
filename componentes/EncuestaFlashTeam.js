...
export default class EncuestaFlashTeam extends Component{
  ...
  descargarCampos=async()=>{
    ...
    try {
      ...
      response=await fetch(URL+"profit_supermercadosflashteam/"+datosUsuario.identificador);
      responseJSON=await response.json();
      camposIniciales=responseJSON[0].supermercados;

      /*Descargar los campos completados*/
      responseCompletados=await fetch(URL+"profit_supermercadosCompletados/"+datosUsuario.identificador+"/FlashTeam/FlashTeam");
